...
const CSS = `
  ...
  .fr-root {
    ...
    background-image: radial-gradient(#171719 2px, transparent 2.5px);
  ...
  .fr-page-title { font-family: 'Luckiest Guy', cursive; font-size: clamp(32px, 5vw, 48px); color: #FF3D81; margin: 0 0 6px; text-shadow: 4px 4px 0px #171719; -webkit-text-stroke: 2px #171719; letter-spacing: 2px; }
  .fr-page-sub { font-family: 'Baloo 2', sans-serif; font-weight: 800; font-size: 18px; color: #171719; margin: 0; background: #FFD23F; display: inline-block; padding: 4px 16px; border: 3px solid #171719; border-radius: 8px; transform: rotate(-2deg); box-shadow: 4px 4px 0px 0px #171719; }

  /* GHOST BUTTON */
  .fr-ghost-btn {
    display: flex; align-items: center; gap: 10px;
    padding: 12px 24px;
    background: #FFF; border: 4px solid #171719; border-radius: 12px;
    font-family: 'Luckiest Guy', cursive; font-size: 18px;
    color: #171719; cursor: pointer; transition: all 0.1s;
    box-shadow: 4px 4px 0px 0px #171719;
  }
  .fr-ghost-btn.fr-ghost-on { background: #3FE0FF; }
  .fr-ghost-btn:hover { transform: translate(-2px, -2px); box-shadow: 6px 6px 0px 0px #171719; }
  .fr-ghost-btn:active { transform: translate(2px, 2px); box-shadow: 0px 0px 0px 0px #171719; }

  /* STATUS BAR */
  .fr-status-bar { display: flex; align-items: center; gap: 20px; margin-bottom: 32px; padding: 16px 20px; background: #FFF; border: 4px solid #171719; border-radius: 16px; flex-wrap: wrap; box-shadow: 8px 8px 0px 0px #171719; }
  .fr-status-item { display: flex; align-items: center; gap: 8px; font-family: 'Luckiest Guy', cursive; font-size: 16px; color: #171719; }
  .fr-status-dot { width: 14px; height: 14px; border-radius: 50%; border: 3px solid #171719; }
  .fr-status-dot.fr-sd-online  { background: #00C853; }
  .fr-status-dot.fr-sd-warning { background: #FFD23F; }
  .fr-status-dot.fr-sd-error   { background: #FF3D81; }
  ...
  .fr-map-chamber {
    ...
    border: 6px solid #171719; background: #FFF;
    box-shadow: 16px 16px 0px 0px #171719;
  }
  ...
  .fr-map-label {
    ...
    font-family: 'Luckiest Guy', cursive; font-size: 24px; color: #FFF;
    background: #FF3D81; padding: 8px 16px; border: 4px solid #171719; border-radius: 12px;
    box-shadow: 4px 4px 0px 0px #171719; transform: rotate(-3deg);
  }
  .fr-coords-hud {
    position: absolute; bottom: 20px; left: 20px; z-index: 30; pointer-events: none;
    font-family: 'Luckiest Guy', cursive; font-size: 14px; color: #171719;
    background: #FFF; padding: 10px 16px; border: 4px solid #171719; border-radius: 12px;
    box-shadow: 4px 4px 0px 0px #171719;
  }
  .fr-member-hud {
    ...
    background: #FFD23F; padding: 10px 16px; border: 4px solid #171719; border-radius: 12px;
    box-shadow: 4px 4px 0px 0px #171719;
  }

  /* PANEL */
  .fr-panel { display: flex; flex-direction: column; background: #3FE0FF; border: 6px solid #171719; border-radius: 24px; overflow: hidden; box-shadow: 16px 16px 0px 0px #171719; position: relative; }
  .fr-panel-header { padding: 24px; border-bottom: 6px solid #171719; display: flex; align-items: center; justify-content: space-between; background: #FFF; }
  .fr-panel-title { font-family: 'Luckiest Guy', cursive; font-size: 24px; color: #171719; }
  .fr-panel-body { flex: 1; overflow-y: auto; padding: 20px; max-height: 520px; }
  .fr-panel-body::-webkit-scrollbar { width: 8px; }
  .fr-panel-body::-webkit-scrollbar-thumb { background: #171719; border-radius: 4px; }

  /* MEMBER CARD */
  .fr-member-card {
    padding: 16px; background: #FFF; border: 4px solid #171719;
    border-radius: 16px; margin-bottom: 16px; transition: all 0.1s; position: relative;
    box-shadow: 4px 4px 0px 0px #171719;
  }
  .fr-member-card:hover { transform: translate(-2px, -2px); box-shadow: 6px 6px 0px 0px #171719; }
  .fr-member-card.fr-me { background: #FFD23F; }
  ...
  .fr-member-name { font-family: 'Luckiest Guy', cursive; font-size: 18px; color: #171719; margin-bottom: 4px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  ...
  .fr-chip { display: flex; align-items: center; gap: 6px; padding: 4px 10px; background: #FFF; border: 3px solid #171719; border-radius: 8px; font-family: 'Luckiest Guy', cursive; font-size: 12px; color: #171719; }
  .fr-chip.fr-chip-on  { background: #00C853; color: #FFF; }
  .fr-chip.fr-chip-gh  { background: #c084fc; color: #FFF; }

  .fr-empty { padding: 40px; text-align: center; font-family: 'Luckiest Guy', cursive; font-size: 24px; color: #171719; }
  .fr-error-bar { padding: 16px; margin-bottom: 24px; background: #FF3D81; border: 4px solid #171719; border-radius: 12px; color: #FFF; font-family: 'Luckiest Guy', cursive; font-size: 18px; display: flex; alignItems: center; gap: 12px; box-shadow: 4px 4px 0px 0px #171719; }

  /* LEAFLET POPUP OVERRIDE */
  .fr-root .leaflet-popup-content-wrapper { background: #FFF !important; border: 4px solid #171719 !important; border-radius: 16px !important; padding: 0 !important; box-shadow: 8px 8px 0px 0px #171719 !important; }
  .fr-root .leaflet-popup-tip { background: #FFF !important; border: 4px solid #171719 !important; }
...
const myIcon = () => L.divIcon({
  ...
  html: `<div style="width:40px;height:40px;border-radius:50%;background:#FFD23F;border:4px solid #171719;display:flex;align-items:center;justify-content:center;font-family:'Luckiest Guy',cursive;font-size:18px;color:#171719;box-shadow:4px 4px 0px 0px #171719;transform:rotate(-5deg);">ME</div>`,
});

const otherIcon = (init='?', online=false) => L.divIcon({
  className: '', iconSize: [36, 36], iconAnchor: [18, 18], popupAnchor: [0, -18],
  html: `<div style="width:36px;height:36px;border-radius:50%;background:${online ? '#3FE0FF' : '#FFF'};border:4px solid #171719;display:flex;align-items:center;justify-content:center;font-family:'Luckiest Guy',cursive;font-size:16px;color:#171719;box-shadow:4px 4px 0px 0px #171719;">${init}</div>`,
});
...
function PopupContent({ name, isMe, isOnline, isGhost, distance, time }) {
  return (
    <div style={{ padding: '16px', minWidth: 180, textAlign: 'center' }}>
      <div style={{ fontFamily: "'Luckiest Guy',cursive", fontSize: 20, color: '#FF3D81', marginBottom: 8 }}>{name}{isMe ? ' (YOU)' : ''}</div>
      {isGhost ? (
        <div style={{ fontFamily: "'Baloo 2',sans-serif", fontWeight: 700, fontSize: 16, color: '#171719' }}>👻 Ghost Mode</div>
      ) : (
        <>
          <div style={{ display: 'inline-block', padding: '4px 10px', background: isOnline ? '#00C853' : '#F5F5F5', border: '3px solid #171719', borderRadius: 8, fontFamily: "'Luckiest Guy',cursive", fontSize: 14, color: isOnline ? '#FFF' : '#171719', marginBottom: 8 }}>
            {isOnline ? 'ACTIVE' : `SEEN: ${time}`}
          </div>
          {distance && distance !== '--' && <div style={{ fontFamily: "'Baloo 2',sans-serif", fontWeight: 800, fontSize: 14, color: '#171719' }}>📍 {distance} AWAY</div>}
        </>
      )}
    </div>
  );
    ...
    <div className="fr-root">
      ...
      <motion.div className="fr-content" initial={{opacity:0,y:40}} animate={{opacity:1,y:0}} transition={{type: 'spring', bounce: 0.4}}>
        ...
        <div className="fr-status-bar">
          ...
          {isGhostModeOn&&<div className="fr-status-item"><span className="fr-status-dot fr-sd-warning"/><span style={{color:'#171719'}}>GHOST MODE</span></div>}
        </div>
        ...
        <div className="fr-grid">
            ...
            <div className="fr-map-inner" ref={chamberRef}>
              <MapContainer center={center} zoom={13} style={{height:'100%',width:'100%',zIndex:0}} scrollWheelZoom touchZoom dragging zoomControl={false}>
                ...
                {myLocation&&!isGhostModeOn&&(
                  ...
                  <Circle center={[myLocation.lat,myLocation.lng]} radius={120} pathOptions={{color:'#FFD23F',fillColor:'#FFD23F',fillOpacity:0.2,weight:2}}/>
                  <Circle center={[myLocation.lat,myLocation.lng]} radius={300} pathOptions={{color:'#FF3D81',fillColor:'#FF3D81',fillOpacity:0.05,weight:1,dashArray:'4 8'}}/></>
                )}
          ...
          <div className="fr-panel">
            <div className="fr-panel-header"><span className="fr-panel-title">FAMILY ONLINE</span><span className="fr-panel-count" style={{fontFamily:"'Luckiest Guy',cursive", color:'#171719'}}>{members.length+1} TOTAL</span></div>
              ...
              {members.map((m,i)=>(
                <motion.div key={m._id} className={`fr-member-card${m.isGhostModeOn?' fr-ghost-card':''}`} initial={{opacity:0,x:-12}} animate={{opacity:1,x:0}} transition={{delay:0.1+i*0.06}}>
                  <div className="fr-member-card-top">
                    ...
                    <div style={{fontFamily:"'Luckiest Guy',cursive",fontSize:14,color:'#FF3D81'}}>{getDist(m)}</div>
                  </div>
                  <div className="fr-member-stats">
