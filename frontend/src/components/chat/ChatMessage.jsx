            ...
            <AnimatePresence>
              {showOptions && (
                <motion.div initial={{ opacity: 0, x: isMe ? 8 : -8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: isMe ? 8 : -8 }} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <button onClick={() => setShowEmojiPicker(p => !p)} style={{ width: 36, height: 36, borderRadius: '50%', background: '#FFF', border: 'none', cursor: 'pointer', fontSize: 16, boxShadow: '2px 2px 15px 0px rgba(0,0,0,0.45)' }}>😀</button>
                  {isMe && <button onClick={executeDelete} style={{ width: 36, height: 36, borderRadius: '50%', background: '#1E352F', border: 'none', cursor: 'pointer', fontSize: 16, boxShadow: '2px 2px 15px 0px rgba(0,0,0,0.45)', color: '#FFF' }}>🗑️</button>}
                </motion.div>
              )}
            </AnimatePresence>
          </div>
          ...
          {(Object.keys(reactionCounts).length > 0 || (isMe && msg?.seenBy?.length > 1)) && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8, flexDirection: isMe ? 'row-reverse' : 'row' }}>
              {Object.entries(reactionCounts).map(([emoji, count]) => (
                <div key={emoji} onClick={() => handleReact(emoji)} style={{ cursor: 'pointer', background: '#FFF', border: 'none', padding: '4px 10px', borderRadius: 16, fontSize: 14, fontWeight: 700, fontFamily: "'Baloo 2',sans-serif", color: '#3E2723', boxShadow: '2px 2px 15px 0px rgba(0,0,0,0.45)' }}>
                  {emoji} {count > 1 && count}
                </div>
              ))}
