  ...
  return createPortal(
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(23,23,25,0.85)', backdropFilter: 'blur(6px)', padding: 20 }}>
      <motion.div initial={{ scale: 0.8, rotate: -2 }} animate={{ scale: 1, rotate: 2 }} exit={{ scale: 0.8, rotate: -2 }} transition={{ type: 'spring', bounce: 0.5 }}
        style={{ background: '#FFF', border: '6px solid #171719', borderRadius: 24, padding: '40px', maxWidth: '440px', width: '100%', textAlign: 'center', position: 'relative', boxShadow: '16px 16px 0px 0px #171719' }}>
        <button onClick={onClose} style={{ position: 'absolute', top: 16, right: 16, width: 40, height: 40, borderRadius: '50%', background: '#FFD23F', border: '4px solid #171719', color: '#171719', cursor: 'pointer', fontFamily: "'Luckiest Guy',cursive", fontSize: 20, boxShadow: '4px 4px 0px 0px #171719' }}>✕</button>
        <div style={{ fontSize: 60, marginBottom: 16 }}>💣</div>
        <h3 style={{ margin: '0 0 16px', fontFamily: "'Luckiest Guy',cursive", fontSize: '32px', color: '#FF3D81', textShadow: '2px 2px 0px #171719', WebkitTextStroke: '1px #171719' }}>ERASE VAULT?</h3>
        <p style={{ margin: '0 0 32px', color: '#171719', fontFamily: "'Baloo 2', sans-serif", fontWeight: 700, fontSize: '18px' }}>{message}</p>
        <div style={{ display: 'flex', gap: '16px', justifyContent: 'center' }}>
          <motion.button onClick={onClose} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} style={{ flex: 1, background: '#FFF', border: '4px solid #171719', color: '#171719', padding: '14px 0', borderRadius: '12px', fontFamily: "'Luckiest Guy',cursive", fontSize: 20, cursor: 'pointer', boxShadow: '4px 4px 0px 0px #171719' }}>NOPE</motion.button>
          <motion.button onClick={() => { onConfirm(); onClose(); }} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} style={{ flex: 1, background: '#FF3D81', border: '4px solid #171719', color: '#FFF', padding: '14px 0', borderRadius: '12px', fontFamily: "'Luckiest Guy',cursive", fontSize: 20, cursor: 'pointer', boxShadow: '4px 4px 0px 0px #171719' }}>ERASE IT!</motion.button>
        </div>
      </motion.div>
    </div>,
    document.body
...
function VaultRoomPage() {
  ...
  return (
    <div style={{ height: 'calc(100vh - 76px)', display: 'flex', flexDirection: 'column', background: '#FFF6E5', backgroundImage: 'radial-gradient(#171719 2px, transparent 2.5px)', backgroundSize: '20px 20px', fontFamily: "'Baloo 2', sans-serif", position: 'relative', overflow: 'hidden' }}>
      ...
      {/* ── HEADER ── */}
      <header style={{ position: 'relative', zIndex: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 36px', background: '#FFF', borderBottom: '6px solid #171719', boxShadow: '0px 8px 0px 0px rgba(23,23,25,0.1)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <div style={{ width: 48, height: 48, borderRadius: '12px', background: '#FFD23F', border: '3px solid #171719', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '4px 4px 0px 0px #171719', transform: 'rotate(-5deg)' }}>
            <span style={{ fontSize: 24 }}>💬</span>
          </div>
          <div>
            <h1 style={{ margin: 0, fontFamily: "'Luckiest Guy',cursive", fontSize: 24, color: '#FF3D81', letterSpacing: 1, textShadow: '2px 2px 0px #171719', WebkitTextStroke: '1px #171719' }}>
              FAMILY CHAT
            </h1>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
              <span style={{ width: 12, height: 12, borderRadius: '50%', background: isConnected ? '#00C853' : '#FF3D81', border: '3px solid #171719' }} />
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
          {user?.role === 'admin' && (
            <motion.button onClick={() => setShowClearModal(true)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
              style={{ background: '#FF3D81', color: '#FFF', border: '3px solid #171719', borderRadius: 8, padding: '8px 12px', cursor: 'pointer', fontFamily: "'Luckiest Guy',cursive", fontSize: 14, display: 'flex', alignItems: 'center', gap: 8, boxShadow: '4px 4px 0px 0px #171719' }}>
              🗑️ NUKE CHAT
            </motion.button>
          )}

          <div style={{ display: 'flex', alignItems: 'center', gap: 12, background: '#3FE0FF', padding: '6px 16px', border: '3px solid #171719', borderRadius: 12, boxShadow: '4px 4px 0px 0px #171719' }}>
            ...
            <div style={{ display: 'flex' }}>
              {onlineUsers.slice(0, 4).map((ou, idx) => (
                <img key={ou.userId} src={ou.avatar || `https://ui-avatars.com/api/?name=${ou.name}&background=FFD23F&color=171719&bold=true`} alt={ou.name} title={ou.name}
                  style={{ width: 32, height: 32, borderRadius: '50%', border: '2px solid #171719', marginLeft: idx === 0 ? 0 : -10, zIndex: 10 - idx, objectFit: 'cover' }} />
              ))}
            </div>
          </div>
        </div>
      ...
      <AnimatePresence>
        {error && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            style={{ position: 'absolute', top: 80, left: '50%', transform: 'translateX(-50%)', zIndex: 15, background: '#FF3D81', border: '4px solid #171719', borderRadius: 12, color: '#FFF', padding: '12px 24px', fontFamily: "'Luckiest Guy',cursive", fontSize: 16, boxShadow: '6px 6px 0px 0px #171719' }}>
            ⚠️ {error}
          </motion.div>
        )}
      </AnimatePresence>
