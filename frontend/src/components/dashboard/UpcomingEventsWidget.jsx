  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        style={{ position: 'fixed', inset: 0, background: 'rgba(4,6,14,0.88)', backdropFilter: 'blur(14px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 20 }}
      ...
      >
        <motion.div
          ...
          style={{ background: 'rgba(12,16,32,0.98)', border: '1px solid rgba(212,168,80,0.25)', borderRadius: 22, width: '100%', maxWidth: 460, padding: '38px 32px', position: 'relative', boxShadow: '0 40px 100px rgba(0,0,0,0.9)' }}
        >
          ...
          <button onClick={() => setIsModalOpen(false)}
            style={{ position: 'absolute', top: 16, right: 16, width: 30, height: 30, borderRadius: '50%', border: '1px solid rgba(212,168,80,0.22)', background: 'rgba(255,255,255,0.03)', color: 'rgba(212,168,80,0.45)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, transition: 'all 0.2s' }}
          ...
          <div style={{ fontFamily: "'Cinzel',serif", fontSize: 19, fontWeight: 700, color: '#e8c87a', textShadow: '0 0 30px rgba(212,168,80,0.3)', marginBottom: 4 }}>
            📅 Upcoming Events
          </div>
          <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 9, letterSpacing: '2px', color: 'rgba(212,168,80,0.4)', marginBottom: 24 }}>
      ...
      <style>{`@keyframes ltDot{0%,80%,100%{transform:scale(0.6);opacity:0.5}40%{transform:scale(1);opacity:1}}`}</style>
    </AnimatePresence>
  );
}
