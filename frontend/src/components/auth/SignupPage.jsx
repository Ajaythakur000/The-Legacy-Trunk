  ...
  return (
    <div style={{ position: 'relative', width: 90, height: 90, margin: '0 auto 16px', zIndex: 2 }}>
      <motion.div
        ...
        style={{
          width: '100%', height: '100%', borderRadius: '50%', background: '#FFD23F',
          border: '4px solid #171719', boxShadow: '6px 6px 0px 0px #171719',
      ...
      >
        ...
        <div style={{ display: 'none', color: '#171719', fontFamily: "'Luckiest Guy',cursive", fontSize: 26 }}>LT</div>
      </motion.div>
      <div style={{
        position: 'absolute', top: -10, left: -20, background: '#3FE0FF', color: '#171719',
        border: '3px solid #171719', borderRadius: '8px', padding: '4px 8px',
        fontFamily: "'Luckiest Guy', cursive", fontSize: 14, transform: 'rotate(-12deg)', boxShadow: '4px 4px 0px 0px #171719'
      }}>
        NEW!
      </div>
    </div>
...
function ComicInput({ type = 'text', name, placeholder, value, onChange, required, icon, label }) {
  ...
  return (
    <div style={{ textAlign: 'left', marginBottom: '12px' }}>
      {label && <label style={{ display: 'block', fontFamily: "'Luckiest Guy', cursive", fontSize: 14, color: '#171719', marginBottom: 4, letterSpacing: '1px' }}>{label}</label>}
      <div style={{ position: 'relative' }}>
        ...
        <input
          ...
          style={{
            ...
            background: '#FFFFFF', border: '3px solid #171719', borderRadius: '12px',
            color: '#171719', fontFamily: "'Baloo 2',sans-serif", fontWeight: 600, fontSize: 16, outline: 'none', boxSizing: 'border-box',
            boxShadow: focused ? '6px 6px 0px 0px #FFD23F' : '4px 4px 0px 0px #171719',
      ...
      <div style={{ position: 'relative' }}>
        ...
        <select
          ...
          style={{
            ...
            background: '#FFFFFF', border: '3px solid #171719', borderRadius: '12px',
            color: '#171719', fontFamily: "'Baloo 2',sans-serif", fontWeight: 600, fontSize: 16, outline: 'none', boxSizing: 'border-box', appearance: 'none', cursor: 'pointer',
            boxShadow: focused ? '6px 6px 0px 0px #FFD23F' : '4px 4px 0px 0px #171719',
            transform: focused ? 'translate(-2px, -2px)' : 'none', transition: 'all 0.2s ease',
          }}
        >
        ...
        <div style={{ position: 'absolute', right: 16, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none', color: '#171719', fontSize: 14, fontWeight: 'bold' }}>▼</div>
      </div>
    </div>
  );
}
...
function SignupPage() {
  ...
  const iconUser = <svg viewBox="0 0 24 24" fill="none" stroke="#171719" strokeWidth="2"><circle cx="12" cy="8" r="4" /><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7" /></svg>;
  ...
  const iconRole = <svg viewBox="0 0 24 24" fill="none" stroke="#171719" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75" /></svg>;
  const iconKey = <svg viewBox="0 0 24 24" fill="none" stroke="#171719" strokeWidth="2"><circle cx="7.5" cy="15.5" r="4.5" /><path d="M21 2l-9.6 9.6M15 8l2 2" /></svg>;
  const iconHeart = <svg viewBox="0 0 24 24" fill="none" stroke="#171719" strokeWidth="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" /></svg>;
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 20px', position: 'relative', overflow: 'hidden' }}>
      ...
      >
        <div style={{ background: '#FFFFFF', border: '4px solid #171719', borderRadius: '16px', padding: '30px 40px', position: 'relative', boxShadow: '12px 12px 0px 0px #171719' }}>
          
          <StickerLogo />

          <div style={{ textAlign: 'center', marginBottom: 20 }}>
            <h2 style={{ margin: '0 0 4px', fontSize: 32, fontFamily: "'Luckiest Guy',cursive", color: '#FF7B00', textShadow: '3px 3px 0px #171719', WebkitTextStroke: '1px #171719' }}>START A SCRAPBOOK</h2>
            <p style={{ margin: 0, fontWeight: 700, fontSize: 16, color: '#171719' }}>Gather the family. Post the memories.</p>
          </div>

          <AnimatePresence>
            {inviteToken && (
              <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                style={{ background: '#3FE0FF', border: '3px solid #171719', borderRadius: 8, padding: '10px', marginBottom: 16, textAlign: 'center', fontFamily: "'Luckiest Guy',cursive", fontSize: 14, color: '#171719', boxShadow: '4px 4px 0px 0px #171719' }}>
                YOU HAVE A SECRET INVITE!
              </motion.div>
            )}
            {error && (
              <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                style={{ background: '#FF3D81', border: '3px solid #171719', borderRadius: 8, padding: '10px', marginBottom: 16, textAlign: 'center', fontFamily: "'Luckiest Guy',cursive", fontSize: 14, color: '#FFF', boxShadow: '4px 4px 0px 0px #171719' }}>
                ERROR: {error}
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <AnimatePresence>
              {!isAdmin && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ overflow: 'hidden' }}>
                  <div style={{ background: '#FFD23F', border: '3px dashed #171719', borderRadius: 12, padding: 16, marginTop: 10, marginBottom: 10 }}>
                    <div style={{ fontFamily: "'Luckiest Guy',cursive", fontSize: 16, color: '#171719', marginBottom: 10, textAlign: 'center' }}>MEMBER DETAILS</div>
            ...
            <motion.button type="submit" disabled={isFormLoading}
              whileHover={!isFormLoading ? { scale: 1.02 } : {}} whileTap={!isFormLoading ? { scale: 0.95, x: 4, y: 4, boxShadow: '0px 0px 0px 0px #171719' } : {}}
              style={{
                width: '100%', padding: '16px', marginTop: 10,
                background: isFormLoading ? '#ccc' : '#FF7B00', border: '4px solid #171719', borderRadius: 12,
                color: '#171719', fontFamily: "'Luckiest Guy',cursive", fontSize: 20, letterSpacing: 2,
                cursor: isFormLoading ? 'not-allowed' : 'pointer', boxShadow: '6px 6px 0px 0px #171719', transition: 'box-shadow 0.1s, transform 0.1s'
              }}>
              {isFormLoading ? 'PACKING BAGS...' : 'JOIN THE FAMILY!'}
            </motion.button>

            <p style={{ margin: '20px 0 0', fontWeight: 700, fontSize: 15, color: '#171719', textAlign: 'center' }}>
              Already a member? <Link to="/login" style={{ color: '#FF3D81', textDecoration: 'underline', fontFamily: "'Luckiest Guy', cursive", fontSize: 18 }}>LOG IN HERE</Link>
            </p>
          </form>
        </div>
      </motion.div>
