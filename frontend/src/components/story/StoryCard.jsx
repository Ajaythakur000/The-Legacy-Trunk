...
function StoryCard({ story, currentUser, onLike, onComment, onDelete, onEdit }) {
  // ✅ Permissions: only author OR same-circle admin can edit/delete
  const isAuthor = Boolean(
    currentUser?._id && story?.user?._id && String(story.user._id) === String(currentUser._id)
  );

  const storyCircleId =
    typeof story?.originCircleId === 'object' && story?.originCircleId?._id
      ? story.originCircleId._id
      : story?.originCircleId || null;

  const currentUserCircleId =
    typeof currentUser?.activeCircleId === 'object' && currentUser?.activeCircleId?._id
      ? currentUser.activeCircleId._id
      : currentUser?.activeCircleId || null;

  const isCircleAdmin = Boolean(
    currentUser?.role === 'admin' &&
      currentUserCircleId &&
      storyCircleId &&
      String(currentUserCircleId) === String(storyCircleId)
  );
  ...
  const isLikedByMe = Boolean(
    currentUser?._id && story?.likes?.some((id) => String(id) === String(currentUser._id))
  );
  ...
  const getFormattedDates = () => {
    ...
    const postDateStr = postDateObj.toLocaleDateString('en-US', {
      month: 'short',
      day: 'numeric',
      year: 'numeric',
    });
  ...
  const handleDownloadImage = async () => {
    if (exportRef.current) {
      ...
      setTimeout(() => setIsDownloaded(false), 1200);
    }
  };

  const handleDoubleTap = (e) => {
    e.preventDefault();
    if (!isLikedByMe && story?._id && typeof onLike === 'function') {
      onLike(story._id);
    }
  ...
  const renderMediaGrid = () => {
      ...
      return (
        ...
        >
          <video
            controls
            preload="metadata"
            style={{ width: '100%', maxHeight: '500px', borderRadius: '2px', background: '#000' }}
          >
            <source src={images[0]} />
          </video>
        </div>
          ...
          {showShieldAnim && (
            <div
              style={{
                ...
                animation: 'shieldPop 1s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards',
                pointerEvents: 'none',
    ...
    if (count === 2) {
      layoutContent = (
        <div
          onDoubleClick={handleDoubleTap}
          style={{ ...gridStyles, gridTemplateColumns: '1fr 1fr', position: 'relative' }}
        >
          <img
            {...imgProps}
            src={getOptimizedUrl(images[0])}
            alt="media-1"
            style={{ width: '100%', height: '100%', objectFit: 'cover', border: '2px solid #faf9f6' }}
          />
          <img
            {...imgProps}
            src={getOptimizedUrl(images[1])}
            alt="media-2"
            style={{ width: '100%', height: '100%', objectFit: 'cover', border: '2px solid #faf9f6' }}
          />
          {showShieldAnim && (
            <div
              style={{
                ...
                left: '50%',
                transform: 'translate(-50%, -50%)',
                animation: 'shieldPop 1s forwards',
                pointerEvents: 'none',
              }}
            >
              <svg width="100" height="100" viewBox="0 0 24 24" fill="#3b82f6" stroke="rgba(255,255,255,0.5)">
                <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path>
              </svg>
            </div>
          )}
    ...
    >
      ...
      <div
        style={{
          ...
          marginBottom: '24px',
          marginTop: story.isMilestone ? '12px' : '0',
        }}
      >
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            ...
            <button
              onClick={() => {
                ...
                if (window.confirm('Erase this memory?')) onDelete(story._id);
              }}
              style={{
                background: 'transparent',
                color: '#ef4444',
        ...
        <div style={{ background: '#f8fafc', padding: '20px', borderRadius: '8px', border: '1px dashed #cbd5e1' }}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            <button
              ...
              style={{ background: '#0f172a', color: 'white', padding: '8px 16px', border: 'none', cursor: 'pointer' }}
            >
              Save
            </button>
            <button
              onClick={() => setIsEditing(false)}
              style={{ background: 'transparent', border: '1px solid #ccc', padding: '8px 16px', cursor: 'pointer' }}
            >
              Cancel
            </button>
          </div>
      ...
      {!isEditing && (
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <button
              onClick={() => typeof onLike === 'function' && onLike(story._id)}
            ...
            >
              <svg
                ...
                height="24"
                viewBox="0 0 24 24"
                fill={isLikedByMe ? '#2563eb' : 'none'}
                stroke={isLikedByMe ? '#2563eb' : '#64748b'}
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              >
            ...
            <span style={{ fontWeight: '600', color: '#475569', fontSize: '14px', fontFamily: 'system-ui, sans-serif' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            >
              <svg
                ...
                height="24"
                viewBox="0 0 24 24"
                fill={showComments ? '#38bdf8' : 'none'}
                stroke={showComments ? '#38bdf8' : '#64748b'}
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              >
          ...
          <button onClick={handleShare} className="action-icon-btn" title="Pass On Memory">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill={isShared ? '#10b981' : 'none'}
              stroke={isShared ? '#10b981' : '#64748b'}
              strokeWidth="1.5"
              strokeLinecap="round"
              strokeLinejoin="round"
            >
          ...
          {images.length > 0 && (
            ...
            >
              <svg
                ...
                height="24"
                viewBox="0 0 24 24"
                fill={isDownloaded ? '#0f172a' : 'none'}
                stroke={isDownloaded ? '#0f172a' : '#64748b'}
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              >
