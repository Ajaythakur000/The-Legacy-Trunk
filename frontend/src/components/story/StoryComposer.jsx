...
const GLOBAL_STYLES = `
  .sc-textarea { scrollbar-width: thin; scrollbar-color: #171719 transparent; }
  .sc-textarea::-webkit-scrollbar { width: 8px; }
  .sc-textarea::-webkit-scrollbar-thumb { background: #171719; border-radius: 4px; }
  .sc-textarea::placeholder, .sc-input::placeholder { color: rgba(23,23,25,0.4); font-family: 'Luckiest Guy', cursive; }
  ...
  .vault-dp-wrap .react-datepicker__input-container input:focus {
    transform: translate(-2px, -2px); box-shadow: 6px 6px 0px 0px #3FE0FF;
  }
  .react-datepicker { background: #FFF !important; border: 4px solid #171719 !important; font-family: 'Baloo 2', sans-serif !important; font-weight: 700 !important; box-shadow: 8px 8px 0px 0px #171719 !important; border-radius: 12px !important; overflow: hidden; }
  .react-datepicker__header { background: #FFD23F !important; border-bottom: 3px solid #171719 !important; }
  .react-datepicker__current-month, .react-datepicker__day-name, .react-datepicker-time__header { color: #171719 !important; font-family: 'Luckiest Guy', cursive !important; font-size: 14px !important; letter-spacing: 1px !important; }
  .react-datepicker__day { color: #171719 !important; border-radius: 6px !important; font-size: 14px !important; font-weight: 700; }
  .react-datepicker__day:hover { background: #3FE0FF !important; color: #171719 !important; border: 2px solid #171719; }
  .react-datepicker__day--selected { background: #FF3D81 !important; color: #FFF !important; border: 2px solid #171719; }
  .react-datepicker__triangle { display: none !important; }
  .react-datepicker__time-container { border-left: 3px solid #171719 !important; }
  .react-datepicker__time { background: #FFF !important; }
  .react-datepicker__time-list-item { color: #171719 !important; font-weight: 700 !important; }
  .react-datepicker__time-list-item:hover { background: #FFD23F !important; }
  .react-datepicker__time-list-item--selected { background: #FF3D81 !important; color: #FFF !important; }

  .sc-select option { background: #FFF; color: #171719; padding: 10px; font-weight: 700; }
`;
...
function ComicInput({ label, type = 'text', name, value, onChange, placeholder, icon, disabled, as: Tag = 'input', rows, maxLength, style: extraStyle }) {
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      {label && <label style={{ display: 'block', fontFamily: "'Luckiest Guy', cursive", fontSize: 14, color: '#171719', marginBottom: 6 }}>{label}</label>}
      <div style={{ position: 'relative' }}>
        ...
        <Tag
          ...
          style={{ 
            ...
            background: '#FFF', border: '3px solid #171719', borderRadius: 12, 
            color: '#171719', fontFamily: "'Baloo 2',sans-serif", fontWeight: 700, fontSize: 18, 
...
// ─── Comic Button ──────────────────────────────────────────────────────────────
function ComicButton({ children, onClick, type = 'button', disabled, fullWidth, color = '#3FE0FF' }) {
  return (
    <motion.button
      type={type} onClick={onClick} disabled={disabled}
      whileHover={!disabled ? { scale: 1.02 } : {}}
      whileTap={!disabled ? { scale: 0.95, x: 4, y: 4, boxShadow: '0px 0px 0px 0px #171719' } : {}}
      style={{
        width: fullWidth ? '100%' : 'auto',
        padding: '14px 24px', background: disabled ? '#ccc' : color,
        border: '4px solid #171719', borderRadius: 12, color: '#171719',
        fontFamily: "'Luckiest Guy',cursive", fontSize: 20, letterSpacing: 1,
        cursor: disabled ? 'not-allowed' : 'pointer',
        boxShadow: '6px 6px 0px 0px #171719', transition: 'box-shadow 0.1s, transform 0.1s',
...
// ─── Toggle checkbox ──────────────────────────────────────────────────────────
function ComicToggle({ checked, onChange, label, icon, color = '#FFD23F' }) {
  return (
    <label style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', userSelect: 'none' }}>
      <div onClick={onChange} style={{ 
        width: 28, height: 28, borderRadius: 8, border: '3px solid #171719', 
        background: checked ? color : '#FFF', display: 'flex', alignItems: 'center', justifyContent: 'center', 
        boxShadow: checked ? '2px 2px 0px 0px #171719' : '4px 4px 0px 0px #171719',
        transform: checked ? 'translate(2px, 2px)' : 'none', transition: 'all .1s' 
      }}>
        {checked && <span style={{ fontSize: 18, color: '#171719' }}>✓</span>}
      </div>
      <span style={{ fontFamily: "'Luckiest Guy',cursive", fontSize: 16, color: '#171719' }}>
        {icon} {label}
      </span>
    </label>
  );
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' }}>
            ...
            <div style={{ marginTop: 26 }}>
              <ComicButton onClick={handleAutoTitle} disabled={aiLoading || compressing || !content.trim() || !activeCircleId} color="#FFD23F">
                🤖 AI TITLE
              </ComicButton>
            </div>
          </div>
          ...
          <div>
            ...
            {/* AI Toolbar */}
            <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap', background: '#F5F5F5', padding: 12, border: '3px dashed #171719', borderRadius: 12, marginTop: -8 }}>
              <select value={tone} onChange={e => setTone(e.target.value)} disabled={aiLoading || compressing || !activeCircleId} style={{ padding: '10px', borderRadius: 8, border: '3px solid #171719', fontFamily: "'Baloo 2', sans-serif", fontWeight: 700, fontSize: 14, outline: 'none', cursor: 'pointer', background: '#FFF', flex: 1, minWidth: 180 }}>
                <option value="" disabled>✨ Pick an AI Flavor...</option>
                {TONES.map(t => <option key={t.value} value={t.value}>{t.icon} {t.label}</option>)}
              </select>
              <motion.button type="button" onClick={handleEnhanceStory} disabled={aiLoading || compressing || !content.trim() || !activeCircleId || !tone} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} style={{ padding: '10px 16px', background: '#3FE0FF', border: '3px solid #171719', borderRadius: 8, fontFamily: "'Luckiest Guy',cursive", color: '#171719', cursor: 'pointer', boxShadow: '2px 2px 0px 0px #171719' }}>
          ...
          {/* ── MEDIA UPLOAD ── */}
          <div style={{ border: '4px dashed #171719', borderRadius: 16, padding: '24px', background: '#FFD23F' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              <div style={{ fontFamily: "'Luckiest Guy',cursive", fontSize: 20, color: '#171719' }}>📸 PHOTOS (MAX 5)</div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button type="button" onClick={() => setShowCollageMaker(true)} disabled={!activeCircleId || uploading || aiLoading || compressing} style={{ padding: '8px 16px', background: '#FF3D81', border: '3px solid #171719', borderRadius: 8, fontFamily: "'Luckiest Guy',cursive", color: '#FFF', cursor: 'pointer', boxShadow: '2px 2px 0px 0px #171719' }}>
                  ✂️ COLLAGE
                </button>
                <label style={{ padding: '8px 16px', background: '#FFF', border: '3px solid #171719', borderRadius: 8, fontFamily: "'Luckiest Guy',cursive", color: '#171719', cursor: 'pointer', boxShadow: '2px 2px 0px 0px #171719' }}>
                ...
                <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                  {mediaPreviews.map((url, idx) => (
                    <motion.div key={idx} initial={{ scale: 0, rotate: -20 }} animate={{ scale: 1, rotate: idx % 2 === 0 ? 4 : -4 }} exit={{ scale: 0 }}
                      style={{ position: 'relative', width: 100, height: 110, background: '#FFF', padding: '6px 6px 20px', border: '3px solid #171719', borderRadius: 4, boxShadow: '4px 4px 0px 0px #171719' }}>
                      <img src={url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', border: '2px solid #171719' }} />
                      <button type="button" onClick={() => removeFile(idx)} style={{ position: 'absolute', top: -10, right: -10, width: 24, height: 24, borderRadius: '50%', background: '#FF3D81', border: '3px solid #171719', color: '#FFF', cursor: 'pointer', fontFamily: "'Luckiest Guy',cursive" }}>X</button>
                    </motion.div>
                  ))}
                </div>
              )}
          ...
          <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
            <div style={{ flex: 1, minWidth: 250, padding: 16, border: '3px solid #171719', borderRadius: 12, background: isMilestone ? '#3FE0FF' : '#F5F5F5', transition: 'background .3s' }}>
              <ComicToggle checked={isMilestone} onChange={() => setIsMilestone(v => !v)} label="MARK AS MILESTONE" icon="🏆" color="#FFD23F" />
              <AnimatePresence>
                {isMilestone && (
            ...
            <div style={{ flex: 1, minWidth: 250, padding: 16, border: '3px solid #171719', borderRadius: 12, background: isGlobalPublic ? '#FFD23F' : '#F5F5F5', display: 'flex', alignItems: 'center' }}>
              <ComicToggle checked={isGlobalPublic} onChange={() => setIsGlobalPublic(v => !v)} label="PUBLIC TO WORLD" icon="🌍" color="#3FE0FF" />
            </div>
          </div>
          ...
          {showCollageMaker && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <motion.div initial={{ scale: .9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: .9, y: 20 }} transition={{ type: 'spring', bounce: 0.5 }}
                style={{ position: 'relative', width: '100%', maxWidth: 900, maxHeight: '90vh', background: '#FFF', border: '6px solid #171719', borderRadius: 24, boxShadow: '16px 16px 0px 0px #171719', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                <button onClick={() => setShowCollageMaker(false)} style={{ position: 'absolute', top: 16, right: 16, zIndex: 10, background: '#FF3D81', border: '4px solid #171719', color: '#FFF', width: 40, height: 40, borderRadius: '50%', cursor: 'pointer', fontFamily: "'Luckiest Guy',cursive", fontSize: 20, boxShadow: '4px 4px 0px 0px #171719' }}>✕</button>
